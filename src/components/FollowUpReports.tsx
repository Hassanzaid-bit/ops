...
import {
  followUpAreaRows,
  visitFollowUpsCount,
  type FollowUpAreaRow,
} from "@/lib/dashboard";
import { listRecords } from "@/lib/records-store";
import { generateFollowUpsReportFromRecord } from "@/lib/report";
...
export function FollowUpReports() {
  ...
  const rows = useMemo(
    () => followUpAreaRows(filteredRecords),
    [filteredRecords],
  );
    ...
    <div className="mx-auto max-w-6xl px-4 pb-16 pt-4">
      <header className="mb-6">
        ...
        <p className="mt-2 max-w-xl text-base text-[var(--ink-muted)]">
          Areas flagged Follow-up visit required across submitted visits —
          separate from the full IPM service report.
        </p>
      </header>
      ...
      <section className="space-y-2">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-[var(--ink-muted)]">
          Follow-up areas ({visible.length})
        </h2>
            ...
            <tbody>
              ...
              {visible.length === 0 && (
                <tr>
                  ...
                  >
                    No follow-up areas in this range.
                  </td>
                </tr>
              )}
            </tbody>
...
function FollowUpDetailModal({
  ...
  const reportText = useMemo(
    () => generateFollowUpsReportFromRecord(record),
    [record],
  );
  const followUpCount = visitFollowUpsCount(record);
        ...
        <div className="flex items-start justify-between gap-3 border-b border-[var(--line)] px-4 py-3">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-[var(--ink-muted)]">
              {followUpCount} follow-up area
              {followUpCount === 1 ? "" : "s"} on this visit ·{" "}
              {row.technicianName}
            </p>
          </div>
          <button
